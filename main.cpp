...
// ExtraTests
// Test calling rehash with a size parameter
TEST(ExtraTest, rehashWithNewSize) {
  std::mt19937 mt {8'398'121};
  const int N {1'000};
  std::uniform_int_distribution<int> dist {-N, N};
  HashSet h;
  h.maxLoadFactor(1.0);
  ...
  const std::size_t newSize {1'000};
  h.rehash(newSize);
  ASSERT_GE(h.bucketCount(), newSize);
}

// rehash with a small size parameter
// should not cause a max load factor violation
TEST(ExtraTest, rehashWithSmallSize) {
  std::mt19937 mt {3'289'289};
  const int N {1'000};
  std::uniform_int_distribution<int> dist {-N, N};
  HashSet h;
  h.maxLoadFactor(1.0);
  const std::size_t numToInsert {1'000};
  for (std::size_t i = 0; i < numToInsert; ++i) {
    h.insert(dist(mt));
  }
  const std::size_t newSize {10};
  h.rehash(newSize);
  ASSERT_GE(h.bucketCount(), numToInsert);
}

// Reduce the load factor by a factor of ten
// and check that the number of buckets is 
// sufficiently increased
TEST(ExtraTest, increaseBucketsTenfold) {
  std::mt19937 mt {183'838};
  const int N {1'000};
  std::uniform_int_distribution<int> dist {-N, N};
  HashSet h;
  h.maxLoadFactor(1.0);
  ...
  h.maxLoadFactor(threshold);
  ASSERT_LE(h.loadFactor(), threshold);
  const float thresholdReciprocal {static_cast<float>(1.0) / threshold};
  const std::size_t minBuckets 
      {numToInsert * static_cast<std::size_t>(thresholdReciprocal)};
  ASSERT_GE(h.bucketCount(), minBuckets);
}

// before an element is inserted it should be checked
// if doing so would violate the max load factor.
TEST(ExtraTest, twoElementsLoadFactor) {
  HashSet h;
  h.maxLoadFactor(0.1);
  h.insert(5);
  ASSERT_GE(h.bucketCount(), 10ul);
}

// Another test going after the same issue.  Make
// sure it is checked before an element added if 
// doing so requires a rehash
TEST(ExtraTest, thirteenElementsLoadFactor) {
  HashSet h;
  h.maxLoadFactor(1.0);
  for (int i = 0; i < 13; ++i) {
    h.insert(i);
  ...
  h.insert(13);
  ASSERT_GE(h.bucketCount(), 13ul);
}

TEST(ExtraTest, sixElementsLoadFactor) {
  HashSet h;
  h.maxLoadFactor(0.1);
  for (int i = 0; i < 6; ++i) {
    h.insert(i);
  }
  ASSERT_GE(h.bucketCount(), 60ul);
}

// test trying to flush out those solutions
// where incrementing an iterator is not constant time
TEST(ExtraTest, timeToIncrementIterator) {
  HashSet h;
  h.maxLoadFactor(1.0);
  h.rehash(1'000'000);
  ASSERT_GE(h.bucketCount(), 1'000'000ul);
  h.insert(1);
  h.insert(1'000'000);
  for (int i = 0; i < 1'000; ++i) {
    auto it = h.begin();
